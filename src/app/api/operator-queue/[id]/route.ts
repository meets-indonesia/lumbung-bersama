    ...
    return Response.json(
      {
        ...
        message: "Akun operator belum tersambung ke workspace koperasi untuk memperbarui antrean.",
      },
      { status: 409 },
    );
  }
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!cooperativeId) {
    return Response.json(
      {
        error: "COOPERATIVE_SCOPE_REQUIRED",
        message: "Akun operator belum tersambung ke workspace koperasi untuk membuat tindak lanjut.",
      },
      { status: 409 },
    );
  }
