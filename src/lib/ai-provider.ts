...
const DEFAULT_MODEL = "gpt-5.5-medium";
...
function buildWaReplyPrompt(input: WaReplyProviderInput) {
  ...
  return [
    ...
    "Format WA natural: sapaan singkat, jawaban utama, langkah berikutnya, lalu satu pertanyaan lanjutan yang spesifik. Tanpa markdown tabel.",
    "",
    "Konteks internal, jangan ditulis di jawaban:",
    `- Rute kemampuan: ${input.agentName}`,
    `- Tugas: ${input.agentJob}`,
    `- Modul internal: ${input.module}`,
    `- Payload: ${input.payloadType}`,
    `- Review mode: ${input.reviewMode}`,
    `- Queue: ${input.queueId ? "ada tindak lanjut" : "tidak ada"}`,
